 ...
 */
private[akka] class CachingConfig(_config: Config) extends Config(_config.cfg) {

  import CachingConfig._

  private val (config: Config, entryMap: ConcurrentHashMap[String, String]) = _config match {
    case cc: CachingConfig ⇒ (cc.config, cc.entryMap)
    case _                 ⇒ (_config, new ConcurrentHashMap[String, String])
  }

  override def withFallback(other: Config) = new CachingConfig(config.withFallback(other))

  override def getString(path: String) = {
    entryMap.get(path) match {
      case null ⇒
        val res = config.getString(path)
        entryMap.putIfAbsent(path, res)
        res
      case e ⇒ e
    }
  }
